module JsonParser {
  signature class SourceSig;

  signature module MakeJsonReaderSig<SourceSig Source> {
    class JsonReader extends Source {
      string getJson();
    }
  }
   ...
   */
  module Make<SourceSig Source, MakeJsonReaderSig<Source> MakeJsonReader> {
    private newtype TJsonToken =
      MkLeftBracketToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        begin = reader.getJson().indexOf("{") and
        begin = end and
        value = "{"
      } or
      MkRightBracketToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        begin = reader.getJson().indexOf("}") and
        begin = end and
        value = "}"
      } or
      MkLeftSquareBracketToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        begin = reader.getJson().indexOf("[") and
        begin = end and
        value = "["
      } or
      MkRightSquareBracketToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        begin = reader.getJson().indexOf("]") and
        begin = end and
        value = "]"
      } or
      MkWhiteSpaceToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        value = reader.getJson().regexpFind("[\\s\\v\\h]", _, begin) and
        begin + value.length() - 1 = end
      } or
      MkCommaToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        begin = reader.getJson().indexOf(",") and
        begin = end and
        value = ","
      } or
      MkColonToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        begin = reader.getJson().indexOf(":") and
        begin = end and
        value = ":"
      } or
      MkNumberToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        value = reader.getJson().regexpFind("-?[1-9]\\d*(\\.\\d+)?((e|E)?(\\+|-)?\\d+)?", _, begin) and
        begin + value.length() - 1 = end
      } or
      MkStringToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        exists(string literal |
          literal = reader.getJson().regexpFind("(?s)\".*?(?<!\\\\)\"", _, begin) and
          // The string without surrounding quotes.
          value = literal.substring(1, literal.length() - 1) and
          begin + literal.length() - 1 = end
        )
      } or
      MkTrueToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        value = reader.getJson().regexpFind("true", _, begin) and
        begin + value.length() - 1 = end
      } or
      MkFalseToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        value = reader.getJson().regexpFind("false", _, begin) and
        begin + value.length() - 1 = end
      } or
      MkNullToken(int begin, int end, string value, MakeJsonReader::JsonReader reader) {
        value = reader.getJson().regexpFind("null", _, begin) and
        begin + value.length() - 1 = end
      }
    ...
    private class JsonToken extends TJsonToken {
      ...
      MakeJsonReader::JsonReader getReader() {
    ...
    private predicate mkJsonValues(JsonToken first, JsonValueList values, JsonToken last) {
      ...
      or
      exists(JsonValue h, JsonToken valueLast, JsonValueList tail |
        mkJsonValue(first, h, valueLast)
      |
    ...
    class JsonValue extends TJsonValue {
      string toString() {
        ...
        or
        exists(JsonMemberList members | this = MkJsonObject(members, _) |
    ...
    JsonValue parse(MakeJsonReader::JsonReader reader) {
      result.getReader() = reader and
