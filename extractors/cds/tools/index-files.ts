...
const codeqlExe: string = osPlatform === 'win32' ? 'codeql.exe' : 'codeql';
const codeqlExePath: string = resolve(join(quote([process.env.CODEQL_DIST ?? '']), codeqlExe));
...
const autobuildScriptName: string = osPlatform === 'win32' ? 'autobuild.cmd' : 'autobuild.sh';
const autobuildScriptPath: string = resolve(join(
    ...
    responseFiles.forEach(file => {
        ...
        while (dir !== resolve(dir, '..')) {
            ...
            if (existsSync(packageJsonPath)) {
                ...
                const packageJsonData = JSON.parse(rawData) as { 
                    name?: string;
 ...
 */
function recursivelyRenameJsonFiles(dirPath: string): void {
